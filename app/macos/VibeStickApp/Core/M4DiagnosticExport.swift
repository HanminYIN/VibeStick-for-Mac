...
enum M4DiagnosticRedactor {
    ...
    private static let rules: [(NSRegularExpression, String)] = {
        let definitions: [(String, String)] = [
            ...
            (#"(?:file://)?/Users/[^\s\"'<>]+"#, "[REDACTED_PATH]"),
            (#"(?<![A-Za-z0-9])~/(?:[^\s\"'<>]*)"#, "[REDACTED_PATH]"),
            (#"(?<![A-Za-z0-9])/(?:Applications|Library|System|private|tmp|opt|usr|Volumes)(?:/[^\s\"'<>]*)?"#, "[REDACTED_PATH]"),
            (#"(?<![A-Za-z0-9:])/(?:[^\s\"'<>/]+)(?:/[^\s\"'<>/]*)*"#, "[REDACTED_PATH]"),
