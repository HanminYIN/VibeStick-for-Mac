...
M4-4C private device-backup work on 2026-08-15.
...
| Espressif `esptool` 5.3.1 standalone `macos-arm64` archive | Espressif GitHub Release, pinned HTTPS URL, archive/inner-file sizes and SHA-256 values, and Developer ID team `QWXF6GB4AV` | Downloaded and prepared only after separate explicit confirmations in a private local cache; M4-4B enables only offline `version`, while separately confirmed M4-4C admits only ROM-only security, flash-ID, MAC and full-flash reads | GPL-2.0-or-later; `LICENSE` remains in the private prepared directory and neither archive nor extracted files are bundled in the App/DMG | Medium | Keep all identities and the read-only whitelist fixed. Complete real backup acceptance before M4-4D; separately review erase/write/recovery commands before enabling them. |
