...
> 状态：源码门、受控烧录与真人真机主流程验收已完成；联合检查点后的 UI 光学校正已另行授权刷入并通过真人真机复核
>
...
- 成功发送与焦点变化后的安全失败完成真人真机验收；联合检查点形成时，失败原因文案精修只在源码中，尚未再次刷写。
- 检查点后的 UI 收口已在使用者逐次授权下刷入，并在真实 135 × 240 LCD 上完成状态文字居中、圆形观感、核心共圆心和感叹号对齐验收；最终收口阶段没有再次 flash。
- 当前源码以 ESP-IDF 5.5.1 完整构建通过，镜像为 `0x15a340`，3 MiB app 分区剩余 `0x1a5cc0`（55%）。
