...
>
> 状态：本机与真机主链路、真实重新配对、失败恢复及真实地址变化验收完成；已封存为本地提交 `8ced6eb`，未推送或发布
...
本轮 M2 验收门禁均已完成。地址变化测试会同时中断承载 Codex 会话的 Mac 外网连接，因此观察脚本一度失联；自动恢复钩子已在测试结束时将 Wi-Fi 完整切回 DHCP。成果已封存为本地提交 `8ced6eb`；push、Release 和 remotes 仍未改变，也不会由文档更新自动执行。
